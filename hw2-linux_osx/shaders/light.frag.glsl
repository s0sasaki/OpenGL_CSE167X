...
//uniform vec4 lightposn[numLights]; // positions of lights  //sasaki
//uniform vec4 lightcolor[numLights]; // colors of lights    //sasaki
uniform vec4 light0posn; // positions of lights  //sasaki
uniform vec4 light0color; // colors of lights    //sasaki
...
void main (void)
{
    if (enablelighting) {
        ...
        vec3 position = light0posn.xyz / light0posn.w ;
        vec3 direction1 = normalize (position - mypos) ; // no attenuation
        vec3 half1 = normalize (direction1 + eyedirn) ;
        vec4 col1 = ComputeLight(direction1, light0color, normal, half1, diffuse, specular, shininess) ;

        finalcolor = ambient + col1;

        // TODO // ライト10個に対応する必要あり。readfileとかdisplayとかも全部forループを使って修正。
